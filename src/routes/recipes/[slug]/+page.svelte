...
{#key data.recipe.id}
	<div class="space-y-6">
		...
		<div class="flex items-center gap-3">
			...
			<div>
				<h2 class="text-xl font-black text-slate-850 dark:text-slate-100">Rezept bearbeiten</h2>
				<p class="text-xs text-slate-500 dark:text-slate-400">
					Passe Details und Zutaten des Rezepts an
				</p>
			</div>
		</div>
		...
		>
			...
			<div class="space-y-1.5">
				<label
					for="name"
					class="text-xs font-semibold text-slate-550 dark:text-slate-400 uppercase tracking-wider"
					>Rezept-Name</label
				>
				<input
					id="name"
			...
			<div class="space-y-2 pt-1">
				<span
					class="text-xs font-semibold text-slate-550 dark:text-slate-400 uppercase tracking-wider block"
					>Rezeptbild</span
				>
				...
				{#if data.recipe.image_path}
					...
					>
						...
						<button
							...
							class="w-full py-2 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 text-rose-650 dark:text-rose-400 text-xs font-bold rounded-lg transition-colors cursor-pointer"
						>
							Bild entfernen
						</button>
					</div>
				{:else}
					<div
						class="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 p-4 rounded-xl flex flex-col items-center justify-center text-center border-dashed border-slate-350 dark:border-slate-800 relative"
					>
						<svg
